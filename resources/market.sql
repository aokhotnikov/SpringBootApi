...
-- Процедуры
--
CREATE DEFINER=`root`@`localhost` PROCEDURE `select_tovar`(tovar_id int(11) )
BEGIN
	-- ----------------------Запрос в переменную----------------------------
	 DECLARE n varchar(100) DEFAULT ''; -- инициализация переменной
	 select name INTO n from tovar where id = tovar_id; -- запрос в переменную
	 SET n = concat(n,' абракадабра'); -- присвоение каких-то значений переменным
	 SELECT n;

	-- ----------------------2 сп-б объявления переменной----------------------------
	-- SET @z='20';
	-- select @z;

	-- ----------------------Условия----------------------------
	-- if (tovar_id > 0) then
		-- select name from tovar where id = tovar_id;
	-- else
		-- select name from tovar where id = 1;
	-- end if;
...
INSERT INTO `counter` (`id`, `num`) VALUES
(1, 6);

-- --------------------------------------------------------

--
-- Структура таблицы `proba_partitions`
--

CREATE TABLE IF NOT EXISTS `proba_partitions` (
  `name` varchar(30) DEFAULT NULL,
  `my_data` date DEFAULT NULL
) ENGINE=MyISAM DEFAULT CHARSET=utf8
/*!50100 PARTITION BY RANGE ( YEAR(my_data))
(PARTITION p_old VALUES LESS THAN (2008) ENGINE = MyISAM,
 PARTITION p_2008 VALUES LESS THAN (2009) ENGINE = MyISAM,
 PARTITION p_2009 VALUES LESS THAN MAXVALUE ENGINE = MyISAM) */;

--
-- Дамп данных таблицы `proba_partitions`
--

INSERT INTO `proba_partitions` (`name`, `my_data`) VALUES
('a', '1985-01-01'),
('a', '2009-05-01');
...
--
-- Структура таблицы `proba_partitions_innodb`
--

CREATE TABLE IF NOT EXISTS `proba_partitions_innodb` (
  `name` varchar(30) DEFAULT NULL,
  `my_data` date DEFAULT NULL
) ENGINE=InnoDB DEFAULT CHARSET=utf8
/*!50100 PARTITION BY RANGE ( YEAR(my_data))
(PARTITION p_old VALUES LESS THAN (2008) ENGINE = InnoDB,
 PARTITION p_2008 VALUES LESS THAN (2009) ENGINE = InnoDB,
 PARTITION p_2009 VALUES LESS THAN MAXVALUE ENGINE = InnoDB) */;

--
-- Дамп данных таблицы `proba_partitions_innodb`
--

INSERT INTO `proba_partitions_innodb` (`name`, `my_data`) VALUES
('a', '1985-01-01'),
('a', '2009-05-01');
...
--
-- Структура таблицы `table_triger`
--

CREATE TABLE IF NOT EXISTS `table_triger` (
  `id` int(11) NOT NULL AUTO_INCREMENT,
...
--
-- Дамп данных таблицы `table_triger`
--

INSERT INTO `table_triger` (`id`, `name`) VALUES
(1, 'sdfsdf'),
(3, 'asdasdasdasd');

--
-- Триггеры `table_triger`
--
DROP TRIGGER IF EXISTS `table_triger_AINS`;
DELIMITER //
CREATE TRIGGER `table_triger_AINS` AFTER INSERT ON `table_triger`
 FOR EACH ROW begin
